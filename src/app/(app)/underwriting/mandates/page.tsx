...
export default function MandatesPage() {
    const [mandateList, setMandateList] = React.useState<NewBusiness[]>([]);

    React.useEffect(() => {
        const policies = getPolicies();
        const pendingMandates = policies.filter(p => p.onboardingStatus === 'Pending Mandate');
        setMandateList(pendingMandates);
    }, []);
      ...
      <Card>
        <CardContent className="pt-6">
           <Table>
            ...
            <TableBody>
              {mandateList.map((business, index) => (
          ...
          {mandateList.length === 0 && (
            <p className="text-center text-muted-foreground py-4">No policies are currently pending mandate verification.</p>
          )}
        </CardContent>
      </Card>
    </div>
